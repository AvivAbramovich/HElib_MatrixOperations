...
int main(){
    ...
    MatSize square(nslots, nslots);
    PTMatrix PTmat1(square,p), PTmat2(square, p);  //random matrix in size origSize1
    ...
    while(true){
        ...
        cin >> multiplyChar;
        if(multiplyChar == 'Y' || multiplyChar == 'y'){
            toEncMult = true;
            break;
        }
        if(multiplyChar == 'N' || multiplyChar == 'n'){
            toEncMult = false;
            break;
        }
